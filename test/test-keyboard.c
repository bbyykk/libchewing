...
void test_set_keyboard_type()
{
    ...
    keyboard_string = taigi_get_KBString(ctx);
    ...
    taigi_free(keyboard_string);
    keyboard_type = taigi_get_KBType(ctx);
    ...
    for (i = 0; i < ARRAY_SIZE(KEYBOARD_STRING); ++i) {
        ok(taigi_set_KBType(ctx, i) == 0, "return shall be 0");
    ...
    ok(taigi_set_KBType(ctx, -1) == -1, "return shall be -1");
    keyboard_type = taigi_get_KBType(ctx);
    ok(keyboard_type == KEYBOARD_DEFAULT_TYPE, "`%d' shall be `%d'", keyboard_type, KEYBOARD_DEFAULT_TYPE);

    ok(taigi_set_KBType(ctx, ARRAY_SIZE(KEYBOARD_STRING) + 1), "return shall be -1");
    keyboard_type = taigi_get_KBType(ctx);
    ok(keyboard_type == KEYBOARD_DEFAULT_TYPE, "`%d' shall be `%d'", keyboard_type, KEYBOARD_DEFAULT_TYPE);
...
void test_KBStr2Num()
{
    ...
    for (i = 0; i < (int) ARRAY_SIZE(KEYBOARD_STRING); ++i) {
        // XXX: taigi_KBStr2Num shall accept const char *.
        ret = taigi_KBStr2Num(KEYBOARD_STRING[i]);
...
void test_enumerate_keyboard_type()
{
    ...
    ok(taigi_kbtype_Total(ctx) == ARRAY_SIZE(KEYBOARD_STRING),
       "total keyboard_string type shall be %d", ARRAY_SIZE(KEYBOARD_STRING));

    taigi_kbtype_Enumerate(ctx);
    for (i = 0; i < ARRAY_SIZE(KEYBOARD_STRING); ++i) {
        ok(taigi_kbtype_hasNext(ctx) == 1, "shall have next keyboard_string type");
        keyboard_string = taigi_kbtype_String(ctx);
        ok(strcmp(keyboard_string, KEYBOARD_STRING[i]) == 0, "`%s' shall be `%s'", keyboard_string, KEYBOARD_STRING[i]);
        taigi_free(keyboard_string);
    }
    ok(taigi_kbtype_hasNext(ctx) == 0, "shall not have next keyboard_string type");
    keyboard_string = taigi_kbtype_String(ctx);
...
void test_hsu_po_to_bo()
{
    ...
    taigi_set_KBType(ctx, taigi_KBStr2Num("KB_HSU"));
...
void test_et26_po_to_bo()
{
    ...
    taigi_set_KBType(ctx, taigi_KBStr2Num("KB_ET26"));
